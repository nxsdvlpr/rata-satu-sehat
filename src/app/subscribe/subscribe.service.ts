...
export class SubscribeService {
  ...
  async generateTokenClinicWithCache(data: any): Promise<string> {
    // console.log('generateTokenClinicWithCache ', data);
    const authUrl = this.config.get<string>('SATU_SEHAT_URL_AUTH');
  ...
  async createOrganization(
    ...
    //     {
    //       use: 'official',
    //       system: `http://sys-ids.kemkes.go.id/organization/${payload.newData?.ssOrganizationId}`,
    ...
    //   ],
    //   partOf: {
    //     reference: `Organization/${payload.newData?.ssOrganizationId}`,
    //   },
    // };
    ...
    const organizationData = payload.newData?.ssOrganizationId;
    console.log(locationData);
  ...
  async unit(idUnit: any) {
    const units = await this.gqlRequestService.unit({ id: idUnit });
    return units;
  }
  async clinic(idClinic: any) {
    const clinics = await this.gqlRequestService.clinic({ id: idClinic });
    return clinics;
  }
  ...
  async createBulkEncounterApi(payload: RMQBasePayload): Promise<any> {
    ...
    // const clinic = await this.gqlRequestService.clinic({ id: interactions });
        ...
        if (
          ...
          const headers = {
            ...
            Authorization: `Bearer ${
              // await this.generateTokenWithCache()
              await this.generateTokenClinicWithCache(interaction.clinic)
            }`,
          };
          const data = {
            resourceType: 'Encounter',
      ...
      for (const interaction of interactions) {
        ...
        if (
          ...
          const headers = {
            ...
            Authorization: `Bearer ${
              // await this.generateTokenWithCache()
              await this.generateTokenClinicWithCache(interaction.clinic[0])
            }`,
          };
          const data = {
            resourceType: 'Encounter',
  ...
  async createBulkConditionApi(payload: RMQBasePayload): Promise<any> {
    ...
    console.log('--------------------------');
    console.log('createBulkConditionApi .', interactions);
    if (interactions.length > 0) {
      for (const interaction of interactions) {
        const customer = await this.gqlRequestService.customer({
          id: interaction.customerId,
